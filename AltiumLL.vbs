...
Sub DebugLog(msg)
    ...
    logPath = InstalledDir & "\AltiumLL_Debug.txt"
    If fso.FileExists(logPath) Then
        Set logFile = fso.OpenTextFile(logPath, 8)
    Else
...
Sub ProcessCB(filename)
    ...
    Dim loopCounter
    loopCounter = 0
    DebugLog "[START] Processing CB file..."
    ...
    Do Until f.AtEndOfStream
       ...
       ElseIf lineArray(0) = "CreateComponentInLib" Then
          ...
          If SchComponent Is Nothing Then
             ...
             MsgBox "ERROR: SchComponent is Nothing!", vbCritical
             Exit Sub
          End If
          DebugLog "[207] SchComponent created"
          If AddSCH Then
...
Function httpGET(url, username, password)
    ...
    If Err.Number <> 0 Then
       ...
       Set oReq = CreateObject("Msxml2.ServerXMLHTTP.6.0")
       oReq.Open "GET", url, False
       oReq.setTimeouts 10000, 10000, 30000, 30000
       oReq.setRequestHeader "Authorization", "Basic " + Replace(Mid(Base64Encode(username + ":" + password),5),vbLf,"")
       oReq.setRequestHeader "Content-Type", "application/x-www-form-urlencoded; charset=UTF-8"
       oReq.setRequestHeader "User-Agent", "AltiumLibraryLoaderV" & AppVersion
       If chk_Proxy.Checked Then
...
Function AddSchLib(component)
    ...
    If CurrentLib Is Nothing Then
        MsgBox "SCH library not found! Check path to SamacSys.SchLib in settings", vbCritical, "Altium Library Loader"
        AddSchLib = False
        Exit Function
    End If
    ...
    ' ÂÀÆÍÎ: ïðîâåðÿåì, åñòü ëè õîòü îäèí êîìïîíåíò (áèáëèîòåêà ìîæåò áûòü ïóñòîé)
    If LibComp Is Nothing Then
        CurrentLib.SchIterator_Destroy(LibraryIterator)
        AddSchLib = True  ' Áèáëèîòåêà ïóñòàÿ, ìîæíî äîáàâëÿòü
        Exit Function
    End If
    ...
    Do
       ...
       If LibCompNameNext = component Then
          CurrentLib.SchIterator_Destroy(LibraryIterator)
          AddSchLib = False  ' Êîìïîíåíò óæå ñóùåñòâóåò
          Exit Function
       End If
       Set LibComp = LibraryIterator.NextSchObject
       If LibComp Is Nothing Then Exit Do  ' Êîíåö ñïèñêà
       LibCompNameNext = LibComp.LibReference
    Loop Until LibCompNameNext = LibCompNamePrev
    
    CurrentLib.SchIterator_Destroy(LibraryIterator)
    AddSchLib = True  ' Êîìïîíåíò íå íàéäåí, ìîæíî äîáàâëÿòü
End Function
...
Sub ShowImages()
    ...
    ' === 1. NIA?AEA CAA?O?AAI SYM E PCB (i?eaeiaeuiue eia) ===
    If SymPcb = "Y" Then
        img_SYM.Visible = True
        img_PCB.Visible = True

        ' --- I?EAEIAEUIUE EIA AE? SYM ---
        On Error Resume Next
        ...
        xHttp.setTimeouts 5000, 5000, 10000, 10000
        If chk_Proxy.Checked Then
           xHttp.setProxy 2, Trim(txt_Address.Text) & ":" & Trim(txt_Port.Text), ""
        End if
        xHttp.Send

        If Err.Number = 0 And xHttp.Status = 200 Then
            Set bStrm = createobject("Adodb.Stream")
            with bStrm
                 .type = 1
                 .open
                 .write xHttp.responseBody
                 .savetofile ImagePath  & "\SYM_" & partID & ".png", 2
            end with
            bStrm.Close
        ...
        If Err.Number = 0 And xHttp.Status = 200 Then
            ...
            with bStrm
                 ...
                 .open
                 .write xHttp.responseBody
                 .savetofile ImagePath  & "\PCB_" & partID & ".png", 2
            end with
            bStrm.Close
    ...
    ' === 2. IIOII CAA?O?AAI 3D (n a?oaeie eiaiaie ia?aiaiiuo) ===
    If ThreeD = "Y" And Image3D <> "" And Image3D <> "null" Then
        img_3DM.Visible = True

        ' Eniieucoai A?OAEA eiaia ia?aiaiiuo, ?oiau ia eiioeeeoiaaou
        Load3DImage partID, Image3D
    Else
...
' Ioaaeuiay ooieoey aey caa?ocee 3D n oieeaeuiuie eiaiaie ia?aiaiiuo
Sub Load3DImage(partID, imageURL)
    On Error Resume Next

    Dim filePath_3d, xmlhttp_3d, adoStream_3d, fso_3d
    filePath_3d = InstalledDir & "\SamacSys_Images\3D_" & partID & ".png"

    ' Nicaaai iauaeou n a?oaeie eiaiaie
    Set xmlhttp_3d = CreateObject("MSXML2.XMLHTTP")
    Set adoStream_3d = CreateObject("ADODB.Stream")
    Set fso_3d = CreateObject("Scripting.FileSystemObject")

    ' Caa?o?aai
    xmlhttp_3d.Open "GET", imageURL, False
    xmlhttp_3d.setTimeouts 3000, 3000, 5000, 5000
    If chk_Proxy.Checked Then
        xmlhttp_3d.setProxy 2, Trim(txt_Address.Text) & ":" & Trim(txt_Port.Text), ""
    End If
    xmlhttp_3d.Send

    If xmlhttp_3d.Status = 200 Then
        ' Nio?aiyai oaee
        adoStream_3d.Type = 1
        adoStream_3d.Open
        adoStream_3d.Write xmlhttp_3d.responseBody
        adoStream_3d.SaveToFile filePath_3d, 2
        adoStream_3d.Close

        ' I?iaoai caa?oceou a ecia?a?aiea
        If fso_3d.FileExists(filePath_3d) Then
            img_3DM.Picture.LoadFromFile filePath_3d
        End If
    End If

    ' I?enoea
    Set adoStream_3d = Nothing
    Set xmlhttp_3d = Nothing
    Set fso_3d = Nothing
    On Error GoTo 0
End Sub

' Ia?aaio?ee eeeea aey 3D (i?inoie)
Sub img_3DMClick(Sender)
    selectedRow = StringGrid1.Row
    partID = StringGrid1.Cols(6)(selectedRow)

    ' I?inoi ioe?uaaai a?aoca?
    CreateObject("WScript.Shell").Run("https://ad.componentsearchengine.com/viewer/3D.php?partID=" & partID)
End Sub

Sub SleepAndClearMessage(milliseconds)
    ' I?inoay caaa??ea ?a?ac ping
    Dim WshShell
    Set WshShell = CreateObject("WScript.Shell")
...
Sub img_3DMClick(Sender)
    ...
    ' I?iaa?yai, anou ee eieaeuiue oaee
    Dim filePath, fso, fileExists
    Set fso = CreateObject("Scripting.FileSystemObject")
    filePath = InstalledDir & "\SamacSys_Images\3D_" & partID & ".png"
    fileExists = fso.FileExists(filePath)

    If fileExists Then
        Dim fileSize
        fileSize = fso.GetFile(filePath).Size
        lbl_Message.Caption = "3D file: " & fileSize & " bytes. Opening browser..."
    Else
        lbl_Message.Caption = "Opening 3D viewer in browser..."
    End If

    Set fso = Nothing

    ' Iaaieuoay caaa??ea ?oiau oaeaaou niiauaiea
    Sleep 500
...
' I?inoay ooieoey caaa??ee
Sub Sleep(milliseconds)
    Dim WshShell
    Set WshShell = CreateObject("WScript.Shell")
    WshShell.Run "timeout /t " & Int(milliseconds/1000) & " /nobreak", 0, True
    Set WshShell = Nothing
End Sub
